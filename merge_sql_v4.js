...
function addMissingColumns(sql, tableName) {
    ...
    // Fix each row - match ),\n( pattern and add defaults before )
    // Use regex to match row pattern: starts with (, ends with ),\n
    // We need to insert defaults between the last value and the closing )
    
    // Replace pattern: 'last_value'),' with 'last_value', defaults),'
    newSql = newSql.replace(/\),\s*\(/g, `, ${defaultsStr}), (`);`);
    
    // Also handle last row (ends with );
    const lastRowPattern = new RegExp(`(${defaultsStr.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})$`, 'g');
    newSql = newSql.replace(/,\s*\);$/, `, ${defaultsStr});`);
