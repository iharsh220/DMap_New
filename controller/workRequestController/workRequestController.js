...
const getMyTaskRequests = async (req, res) => {
    try {
        ...
        // First, find all issue assignments where the current user is the requester
        const issueAssignments = await IssueAssignments.findAll({
            where: { requested_by_user_id: user_id },
            attributes: ['id', 'task_id', 'issue_id', 'version', 'description', 'status', 'review']
        });
        ...
        if (issueAssignments.length === 0) {
            return res.json({
                success: true,
                data: [],
                pagination: { ...req.pagination, total: 0 },
                message: 'No task requests found'
            });
        }

        // Get unique task IDs
        const taskIds = [...new Set(issueAssignments.map(ia => ia.task_id).filter(tid => tid !== null))];
        ...
        if (taskIds.length === 0) {
            return res.json({
                ...
                data: [],
                pagination: { ...req.pagination, total: 0 },
                message: 'No tasks found for issue assignments'
            });
        }

        // Get the tasks with their work requests
        const tasks = await Tasks.findAll({
            where: { id: { [Op.in]: taskIds } },
            attributes: ['id', 'task_name', 'description', 'work_request_id', 'status', 'deadline']
        });

        // Get unique work request IDs
        const workRequestIds = [...new Set(tasks.map(t => t.work_request_id).filter(wrid => wrid !== null))];
        ...
        let where = { id: { [Op.in]: workRequestIds } };
        ...
        // Get work requests with full details
        const result = await workRequestService.getAll({
            where,
            attributes: { exclude: ['request_type_id', 'updated_at'] },
            include: [
                ...
                },
                {
                    ...
                    include: [
                        ...
                        },
                        {
                            ...
                            as: 'issueAssignments',
                            where: { requested_by_user_id: user_id },
                            attributes: ['id', 'version', 'description', 'status', 'review'],
                            include: [
                                {
        ...
        if (result.success) {
            ...
            res.status(500).json({ success: false, error: result.error, message: 'Failed to retrieve task requests' });
        }
    } catch (error) {
        console.error('Error fetching task requests:', error);
        res.status(500).json({ success: false, error: error.message, message: 'Failed to retrieve task requests' });
    }
};
